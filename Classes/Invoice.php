...
namespace MikailGazi\Invoices\Classes;

use Carbon\Carbon;
use MikailGazi\Invoices\Traits\Setters;
...
class Invoice
{
    ...
    {   $total = $this->subTotalPrice();
        if ($this->discount) {
            $total = $this->subTotalPrice() * ((100 - $this->discount) / 100);
        }
        return bcadd($total, $this->taxPrice(), $this->decimals);
    }
